...
# M1 connector reference: vehapiproxi/API_CONSUMPTION_DOCUMENTATION.md
# Placeholder / virtual article IDs — not fetched via labor endpoint like real L: articles.
SPECIAL_ARTICLE_IDS_SKIP_LABOR = frozenset({"-997", "-998", "-999"})
# MaintenanceSchedulesFacade.searchByFrequency uses per-type requests.
MAINTENANCE_FREQUENCY_TYPE_CODES = ("F", "N", "R")
...
class M1Crawler:
    ...
    def _safe_path_segment(value) -> str:
        """
        Convert API-provided names into safe directory segments.
        Prevents accidental nested directories from model names like "FX35/FX50".
        """
    ...
    @staticmethod
    def _response_items(payload: dict | list | None) -> list:
        """Best-effort extraction of list payloads from M1 responses."""
        ...
        if isinstance(body, dict):
            if isinstance(body.get("items"), list):
                return body["items"]
            if isinstance(body.get("articleDetails"), list):
                return body["articleDetails"]
        if isinstance(payload.get("items"), list):
            return payload["items"]
        return []

    @staticmethod
    def _pick_sample_model(models_list: list, year: int, source: str) -> dict | None:
        """
        Pick a model likely to produce usable data.
        Prefers models that already resolve to a vehicle ID.
        """
        if not models_list:
            return None
        with_id = [m for m in models_list if isinstance(m, dict) and resolve_vehicle_id(m, year, source)]
        if with_id:
            return with_id[0]
        return models_list[0] if isinstance(models_list[0], dict) else None
    ...
    def save_json(self, path_parts, filename, data):
        safe_path_parts = [self._safe_path_segment(p) for p in path_parts]
        target_dir = os.path.join(self.output_dir, *safe_path_parts)
    ...
    async def crawl_vehicle_data(self, year, make_name, model, source):
        async with self.semaphore:
            ...
            if articles:
                ...
                # Fallback to dedicated endpoints if articleDetails extraction misses a silo.
                if not silos["procedures"]:
                    await self._safe_delay()
                    silos["procedures"] = self._response_items(
                        await self.client.get_procedures(source, vehicle_id, motor_vehicle_id)
                    )
                if not silos["wiring_diagrams"]:
                    await self._safe_delay()
                    silos["wiring_diagrams"] = self._response_items(
                        await self.client.get_wiring(source, vehicle_id, motor_vehicle_id)
                    )
                if not silos["component_locations"]:
                    await self._safe_delay()
                    silos["component_locations"] = self._response_items(
                        await self.client.get_components(source, vehicle_id, motor_vehicle_id)
                    )
                if silos["procedures"]:
                    self.save_json(
                ...
                labor_articles = [
                    a
                    for a in silos["procedures"]
                    if str(a.get("id") or "").upper().startswith("L:")
                    and str(a.get("id") or "") not in SPECIAL_ARTICLE_IDS_SKIP_LABOR
                ]
                for proc in labor_articles[: self.max_labor_per_vehicle]:
                    aid = proc.get("id")
                    if not aid or str(aid) in SPECIAL_ARTICLE_IDS_SKIP_LABOR:
                        continue
                    await self._safe_delay()
            ...
            # 3b. DTCs & TSBs (same vehicle routes as M1 asset APIs / vehapi dashboard)
            await self._safe_delay()
            dtcs = await self.client.get_dtcs(source, vehicle_id, motor_vehicle_id)
            if dtcs:
                self.save_json(
                    [str(year), make_name, model_name], "dtcs.json", dtcs
                )

            await self._safe_delay()
            tsbs = await self.client.get_tsbs(source, vehicle_id, motor_vehicle_id)
            if tsbs:
                self.save_json(
                    [str(year), make_name, model_name], "tsbs.json", tsbs
                )

            # 4. Maintenance (indicators + frequency by type + intervals)
            # Matches M1 MaintenanceSchedulesFacade: indicators, searchByFrequency (F/N/R), searchByInterval.
            await self._safe_delay()
            indicators = await self.client.get_maintenance_indicators(
                source, vehicle_id, motor_vehicle_id
            )
            if indicators:
                self.save_json(
                    [str(year), make_name, model_name],
                    "maintenance_indicators.json",
                    indicators,
                )

            freq_by_code: dict = {}
            for code in MAINTENANCE_FREQUENCY_TYPE_CODES:
                await self._safe_delay()
                freq = await self.client.get_maintenance_frequency(
                    source,
                    vehicle_id,
                    motor_vehicle_id,
                    frequency_type_code=code,
                )
                if freq:
            ...
            await self._safe_delay()
            # One interval sample for data mapping (Miles 5000)
            ...
            for make in tqdm(makes, desc=f"Makes in {year}"):
                ...
                if models_data:
                    ...
                    if source and models_list:
                        ...
                        sample_model = self._pick_sample_model(models_list, year, source)
                        if not sample_model:
                            continue
