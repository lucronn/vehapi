...
- **Open (Medium)** — **DTC/TSB ingest — HTML image OCR:** When article text lives in embedded images, parse may still yield thin rows until HTML image OCR exists (PDF path already covered).
...
| **High** | **Maintenance ingest:** Interval/frequency Motor body parsing fixed (`maintenance-response.util.ts` + `DataSyncService`). Optional follow-up: resolve schedule/application IDs to OEM labels if Motor exposes a detail endpoint. |
| **High** | **DTC/TSB — HTML image OCR:** Optional follow-up to routing fix (2026-03-26): OCR non-PDF inline images so parsed `dtcs` / `tsbs` rows are richer when the body is bitmap-heavy. |
