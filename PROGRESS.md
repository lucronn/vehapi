...
**Last updated**: 2026-03-22 — **Normalization wrap-up (remaining silos):** added additive schema/runtime support for **`diagram_document`**, **`component_location_document`**, and **`labor_operation`**; the worker now normalizes diagram/component-location article HTML and labor detail payloads into those tables, direct `/graphic/*` binary responses now persist **`media_asset`** metadata, article cache reads include the new normalized tables, and the article viewer now uses the **labor endpoint** for `L:` article ids. Follow-up traceability fix: article parse paths now also create **`evidence_link` → `content_item`** rows so arbitrary article verification is not limited to procedure/DTC/TSB-specific entity tables. Remaining release work is now mostly **target-runtime golden-vehicle verification**, not missing runtime coverage.
...
| Medium | Run and record a golden-vehicle normalization verification pass with `cd vehapiproxi && npm run verify:golden-vehicles -- --local` (or remote equivalent), then link the generated report here or in `documentation/RELEASE_CHECKLIST.md`. Current repo code fixes the `content_item` traceability gap; local verification in this workspace is still blocked by mixed-runtime dependencies (`canvas` binary mismatch under WSL/Node 18 tooling), so final signoff should use the target Node 22 runtime. |
