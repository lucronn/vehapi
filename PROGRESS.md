...
**Last updated**: 2026-03-20 (CI hardened to Node24 action runtime; `.gitignore`: `.cursor/settings.json`; repo cleanup to `randdev/`; routes refactor + Nemotron + phase-1 normalization.) DDL `documentation/migrations/20260319_phase1_normalization.sql` + `supabase_schema.sql` tables `canonical_bucket`, `evidence_ingest`, `evidence_link`, `bucket_alias`, `content_item`; `npm run migrate:phase1`; worker dual-write + catalog `evidence_ingest`; non-catalog payload summaries in `evidence_ingest`; `pdf_native_text.js` for PDF-only articles + guarded/tunable vision fallback (`ENABLE_NEMOTRON_PDF_VISION_FALLBACK`, `MIN_NATIVE_PDF_TEXT_LENGTH`, `PDF_VISION_FALLBACK_PAGE`); lazy `content_item` enrichment updates (`display_description`, `search_text`, enrichment metadata) from parsed article content; **new** `evidence_link` insertion for parsed rows (`procedures`/`dtcs`/`tsbs`/`specifications`) via `external_id`.
...
- [x] **Phase 1 verification (2026-03-20)** — added `vehapiproxi/scripts/verify-evidence-links-one-article.js` (`npm run verify:evidence-links`) to assert one-article pipeline creates `content_item` enrichment + `evidence_ingest` + `evidence_link`.
