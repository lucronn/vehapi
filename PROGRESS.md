...
- **Open (High)** — **L2 embedding not populating:** `content_chunk` remains empty and L2 evidence retrieval does not work for affected vehicles. Known failure mode: embedding requests missing the embedding API `input_type` parameter required for asymmetric models (400). Expected: `vehapiproxi/src/embedding_client.js` must send correct `input_type` ('query'/'passage') and embeddings ingest should write `content_chunk`.
...
| **High** | **L2 embeddings (`input_type`):** Update embedding client to always pass required embedding `input_type` ('query'/'passage') and ensure `content_chunk` writes succeed. |
