...
| Priority | Task |
|----------|------|
| **High** | **Apply DB migrations on staging/prod:** `npm run migrate:rls-tightening` then `npm run migrate:match-content-chunks-rpc` (after L2 tables exist). **Prod:** set `environment.features.l2Search` when ready. Remaining plan: **`media_asset`** ingest wiring (Task 7), **`documentation/RELEASE_CHECKLIST.md`** (Task 8). See `docs/plans/2026-03-21-production-readiness-paid-plus-l2.md`. |
