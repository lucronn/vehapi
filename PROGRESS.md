...
- **Next (code):** Finish rollout verification on real target data: apply additive migrations, run golden vehicles on staging/prod if needed, and confirm normalized diagram/component-location/labor reads behave in the deployed environment. L1 **`spec_fact`**, **`maintenance_task`**, **`procedure_step`**, **`procedure_tool`**, **`procedure_part`**, **`diagram_document`**, **`component_location_document`**, **`labor_operation`** + L2 **DDL/query/ingest** are now in repo.
