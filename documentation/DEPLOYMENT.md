...
**Sanity check:** `POST /api/rewrite` with `{ "html": "<p>test</p>" }` — with a valid key you should get `200` and rewritten HTML; `503` with `code: "MISSING_LLM_KEY"` means the key is not in the function environment; `code: "AI_MODULE_LOAD_FAILED"` means `vehapiproxi` failed to load `ai_parser`. A frequent cause: dependencies only listed in `vehapiproxi/package.json` — Vercel installs **root** `package.json` for the serverless function; `ai_parser` → `html_preprocess` requires **`cheerio`** and **`turndown`** at the repo root. Check function logs for `ERR_MODULE_NOT_FOUND`.
