#!/usr/bin/env node
/**
 * Apply additive phase-1 normalization DDL (no wipe).
 * File: documentation/migrations/20260319_phase1_normalization.sql
 *
 * Requires SUPABASE_DB_URL or SUPABASE_URL + SUPABASE_DB_PASSWORD (same as run-migrate-schema.js).
 *
 *   cd vehapiproxi && npm run migrate:phase1
 */
...
const sqlPath = path.join(repoRoot, 'documentation', 'migrations', '20260319_phase1_normalization.sql');
...
async function run() {
    ...
    try {
        ...
        console.log('Applying phase-1 normalization (additive)...');
