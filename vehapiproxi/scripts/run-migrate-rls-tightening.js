#!/usr/bin/env node
/**
 * Apply documentation/migrations/20260321_rls_staging_tightening.sql
 *   cd vehapiproxi && npm run migrate:rls-tightening
 */
import pg from 'pg';
...
const sqlPath = path.join(repoRoot, 'documentation', 'migrations', '20260321_rls_staging_tightening.sql');
...
async function run() {
    ...
    try {
        ...
        console.log('Applying RLS tightening migration...');
        await client.query(sql);
        console.log('RLS tightening migration completed.');
    } catch (err) {
        console.error(err);
        process.exit(1);
    } finally {
