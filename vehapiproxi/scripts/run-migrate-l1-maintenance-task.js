#!/usr/bin/env node
/**
 * Apply additive L1 maintenance_task DDL.
 * File: documentation/migrations/20260321_l1_maintenance_task.sql
 *
 *   cd vehapiproxi && npm run migrate:l1-maintenance-task
 */
...
const sqlPath = path.join(repoRoot, 'documentation', 'migrations', '20260321_l1_maintenance_task.sql');
...
async function run() {
    ...
    try {
        ...
        console.log('Applying L1 maintenance_task migration (additive)...');
        await client.query(sql);
        console.log('L1 maintenance_task migration completed.');
    } catch (err) {
