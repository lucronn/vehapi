...
export class HtmlProcessingService {
   ...
   */
  processHtmlContent(html: string, baseUrl: string, contentSource?: string, vehicleId?: string): string {
    if (!html) return '';
    ...
    html = html.replace(/<mtr-image\s+([^>]*)\/?>/gi, (match, attrs) => {
      ...
      const graphicUrl = contentSource
        ? `${baseUrl}/api/source/${contentSource}/graphic/${id}`
        : `${baseUrl}/graphic/${id}`;
    ...
    processed = processed.replace(/href\s*=\s*("[^"]*"|'[^']*'|[^\s>]+)/gi, (match, urlWithQuotes) => {
      ...
      if (url.startsWith('http://') || url.startsWith('https://')) {
        ...
        if (url.includes(baseUrl) && url.includes('#/')) {
