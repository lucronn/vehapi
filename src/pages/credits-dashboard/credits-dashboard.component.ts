  ...
  template: `
    <div class="min-h-screen bg-paper text-ink font-sans">
      ...
      <div class="fixed inset-0 pointer-events-none">
        <div class="absolute top-0 left-0 w-full h-[600px] bg-gradient-to-b from-accent/5 to-transparent"></div>
        <div class="absolute bottom-0 right-0 w-[600px] h-[600px] bg-accent/5 rounded-full blur-[120px]"></div>
      ...
      <div class="relative z-10 max-w-5xl xl:max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10"
          ...
          <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
            @if (authService.user(); as user) {
              ...
              <div class="flex items-center gap-2">
                ...
                <button (click)="openAuthModal('signup')" class="flex items-center gap-2 px-4 py-3 min-h-[44px] rounded-xl bg-accent-soft border border-accent/40 text-accent hover:bg-accent/20 text-sm font-medium touch-manipulation">
        ...
        <div class="mb-6 flex items-center gap-3 bg-accent-soft border border-accent/30 rounded-xl px-5 py-4 text-accent">
          <span class="inline-block w-5 h-5 border-2 border-accent/30 border-t-accent rounded-full animate-spin" aria-hidden="true"></span>
        ...
        <div class="space-y-6 animate-fade-in-up">
          ...
          <div class="bg-surface-soft border border-hairline rounded-xl p-4 sm:p-6">
            ...
            <p class="text-xs text-faint mt-4">
              <button (click)="openBillingPortal()" [disabled]="creditsService.portalLoading()"
                class="text-accent hover:underline disabled:opacity-50">Manage payment methods and invoices</button>
            </p>
        ...
        <div class="animate-fade-in-up">
          ...
          <div class="flex flex-col items-center justify-center py-20 text-center">
            <div class="w-16 h-16 bg-surface-soft rounded-2xl flex items-center justify-center mb-4">
              <lucide-icon [img]="icons.Car" class="w-8 h-8 text-faint"></lucide-icon>
            </div>
            <h3 class="text-lg font-semibold mb-2">No vehicles unlocked yet</h3>
            <p class="text-faint text-sm max-w-xs mb-6">
              Vehicles you unlock will appear here.
            </p>
            <a routerLink="/" class="px-5 py-2 bg-accent text-accent-ink text-sm font-bold rounded-full hover:bg-accent/90 transition-colors">
              Search Vehicles
            </a>
          </div>
          } @else {
          <div class="space-y-4">
            ...
            <div class="bg-surface-soft border border-hairline rounded-xl p-5 hover:border-hairline transition-colors">
              <div class="flex items-start justify-between gap-4">
                <div class="flex items-center gap-4">
                  <div class="w-10 h-10 bg-accent-soft rounded-xl flex items-center justify-center flex-shrink-0">
                    <lucide-icon [img]="icons.Car" class="w-5 h-5 text-accent"></lucide-icon>
                  </div>
                  <div>
                    <p class="font-semibold">{{ vehicle.vehicleName || vehicle.vehicleId }}</p>
                    <p class="text-xs text-faint mt-0.5">ID: {{ vehicle.vehicleId }}</p>
                  </div>
                </div>
                <div class="flex flex-wrap gap-2 justify-end">
                  @for (mod of vehicle.modules; track mod) {
                  <span class="text-xs bg-accent-soft text-accent border border-accent/20 px-2.5 py-1 rounded-full font-medium">
                    {{ moduleLabel(mod) }}
                  </span>
                  }
                </div>
        ...
        <div class="animate-fade-in-up">
          <p class="text-xs text-faint mb-4">Purchases and usage history.</p>
          @if (creditsService.transactionsLoading()) {
          <div class="space-y-3">
            @for (i of [1,2,3,4,5]; track i) {
            <div class="h-16 bg-surface-soft rounded-xl animate-pulse"></div>
            }
          </div>
          } @else if (creditsService.transactions().length === 0) {
          <div class="flex flex-col items-center justify-center py-20 text-center">
            <div class="w-16 h-16 bg-surface-soft rounded-2xl flex items-center justify-center mb-4">
              <lucide-icon [img]="icons.Receipt" class="w-8 h-8 text-faint"></lucide-icon>
            </div>
            <h3 class="text-lg font-semibold mb-2">No transactions yet</h3>
            <p class="text-faint text-sm">Your transactions will appear here.</p>
          </div>
          } @else {
          <!-- Mobile: card layout -->
          <div class="sm:hidden space-y-3">
            @for (txn of creditsService.transactions(); track txn.id) {
            <div class="bg-surface-soft border border-hairline rounded-xl p-4 touch-manipulation">
              ...
              <div class="flex items-center gap-2 mt-2">
                ...
                @if (txn.usd_cents) {
                <span class="text-xs text-faint">\${{ txn.usd_cents / 100 | number:'1.2-2' }}</span>
                }
              </div>
            </div>
            }
          ...
          <!-- Desktop: table -->
          <div class="hidden sm:block bg-surface-soft border border-hairline rounded-xl overflow-hidden">
            <table class="w-full text-sm">
              <thead>
                <tr class="border-b border-hairline">
                  <th class="text-left text-xs text-faint uppercase tracking-wider px-5 py-3">Date</th>
                  <th class="text-left text-xs text-faint uppercase tracking-wider px-5 py-3">Type</th>
                  <th class="text-left text-xs text-faint uppercase tracking-wider px-5 py-3">Description</th>
                  <th class="text-right text-xs text-faint uppercase tracking-wider px-5 py-3">Amount</th>
                  <th class="text-right text-xs text-faint uppercase tracking-wider px-5 py-3">USD</th>
                </tr>
              </thead>
              <tbody>
                @for (txn of creditsService.transactions(); track txn.id) {
