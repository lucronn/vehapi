    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in"
      ...
      <div class="glass-card max-w-2xl w-full max-h-[80vh] overflow-hidden animate-scale-in" (click)="$event.stopPropagation()">
        ...
        <div class="px-6 py-4 flex items-center justify-between border-b border-white/5">
          <div>
            <p class="text-[10px] font-mono uppercase tracking-wider text-torque-cyan mb-0.5">Configuration</p>
            <h2 class="text-lg font-heading font-bold text-white">Select Vehicle Model</h2>
          </div>
        ...
        <div class="px-6 py-3 border-b border-white/5 bg-white/[0.02]">
          <p class="text-sm text-torque-secondary">
        ...
        <div class="overflow-y-auto max-h-[50vh]">
          @for (option of options; track option.id) {
            <button
              ...
              class="w-full px-6 py-4 flex items-center justify-between transition-all group hover:bg-white/[0.03] min-h-[52px]"
              style="border-bottom:1px solid rgba(255,255,255,0.04)">
              <div class="flex-1 text-left min-w-0">
                <p class="text-base font-medium text-white group-hover:text-torque-cyan transition-colors truncate">
                  {{ option.displayName }}
                </p>
                @if (option.qualifier) {
                  <p class="text-xs mt-0.5 text-torque-muted">{{ option.qualifier }}</p>
                }
              </div>
              <lucide-icon [img]="icons.ChevronRight"
                class="w-5 h-5 group-hover:translate-x-1 transition-all text-torque-muted flex-shrink-0"></lucide-icon>
            </button>
          }
        </div>
      </div>
